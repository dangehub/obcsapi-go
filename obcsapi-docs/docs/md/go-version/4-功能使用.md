...
一些访问方式可见文件 [http/server.http](https://github.com/dangehub/obcsapi-go/blob/master/http/server.http) 或者运行后见 Swagger ，路径 `http://localhost:8900/swagger/index.html`
...
最后一条，可以接入百度unit，获取更加智能的回复。示例脚本在 [https://github.com/dangehub/obcsapi-go/tree/master/server/sh](https://github.com/dangehub/obcsapi-go/tree/master/server/sh) 有提供，分别演示了获取最后一次输出，本地获取 token 内容，演示调用其他脚本等。使用 Bash 脚本，可以自定义各种各样的能力。如有 Bash 脚本编程问题，建议使用 ChatGPT 或者 New Bing 生成脚本，然后修改使用。
